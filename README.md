...
Built with React 19 + Vite, React Router 7, Firebase (Auth, Firestore, Storage), MediaPipe face landmarking for the photo try-on (Three.js / react-three-fiber power the phase-2 3D mode, currently gated off), and the in-repo [`visionkart-backend`](../visionkart-backend) on Render for CCAvenue payments and order email.
...
| `paymentBackendUrl` | Base URL for the CCAvenue payment / invoice backend (`../visionkart-backend`, hosted on Render). Every call into that backend should be derived from this — don't hardcode the URL. |
| `enable3DTryOn` | Phase flag. `false` (phase 1) ships the 2D photo try-on only; `true` re-enables the live-AR/3D entry points (`@vto/sdk`, VTOCanvas). |

Firebase config is currently inlined in `src/firebase.config.js`. Firebase client keys are not secrets, but the project still needs **Firestore security rules** and **Storage security rules** to be locked down for production — the rules live in this repo at `../admin-portal/firestore.rules` and `../admin-portal/storage.rules` (deployed with the Firebase CLI; see `../docs/DEPLOYMENT.md`). The outstanding lockdown work is tracked in `../admin-portal/SECURITY-TODO.md`.
...
- **Try-on.** Phase 1 ships the **2D photo try-on** (`Components/VTO/PhotoTryOn`): MediaPipe FaceLandmarker finds the eyes on a selfie or uploaded photo and composites the product's transparent frame cutout, with size/position sliders. Every entry point (product page banner, `/virtual-try-on` page, wishlist badge) is gated on `hasTryOnImage()` from `src/utils/tryOnModel.js` — a product only offers try-on when the admin has uploaded a dedicated cutout (`tryOnImageUrl`); raw catalog photos are never composited. The camera `MediaStream` is stopped on close/unmount. The 3D/live-AR components (`VTOCanvas`, `@vto/sdk`) are gated behind `config.enable3DTryOn` for phase 2.
...
- The largest hero/category PNGs have been converted to WebP; a sweep of the remaining `src/assets/` images (plus responsive sizes) is still a candidate performance win.
